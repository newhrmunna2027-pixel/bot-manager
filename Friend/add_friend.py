# -*- coding: utf-8 -*-
# Friend/add_friend.py - Send Friend Request (OB54 Patched)
...
def add_target_friend(token, target_uid):
    try:
        ...
        encrypted_payload = bot.encrypt_api(payload)

        region = bot.get_server_from_token(token)
        endpoint = bot.get_base_url(region) + "RequestAddingFriend"
        ...
        res = requests.post(endpoint, headers=headers, data=bytes.fromhex(encrypted_payload), timeout=10, verify=False)
